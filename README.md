...
A Three.js browser prototype for patient-paced movement gameplay. Six alternating close/open prompts trigger jumps and ducks. Every prompt freezes simulation time, character animation, camera and obstacle positions. No time limit or failure penalty. Escape pauses and resumes during travel, prompts or actions. Switching away pauses automatically.
...
## Inputs
F = simulate close hand. O = simulate open hand. Mouse buttons provide equivalent controls. Key repeats and inputs outside the expected prompt are ignored. Future sensors can call `confirmMovement` through the same input boundary. No patient data is collected or saved.

## Validation and limits
State tests cover indefinite pauses, resuming mid-action, incorrect and duplicate inputs, six-prompt completion, and large frame deltas. Browser visual and interaction testing covers forward orientation, keyboard jump, mouse duck, incorrect input, and rest/resume. Hardware frame-rate profiling has not been performed. Balanced graphics caps pixel ratio at 1.25; high caps it at 1.75. Assets are served locally with no runtime third-party asset requests.

This is a playable visual prototype, not a photorealistic final art pass. The environment uses scanned PBR textures and constructed ruins; foliage is simplified and the human model is a temporary Mixamo soldier. Jump/duck motion is procedural and needs bespoke motion-captured animation for a finished realistic result. Clinical efficacy is outside this prototype's scope.
