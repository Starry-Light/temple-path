...
test('running keeps looping for many cycles and resumes after jump and slide',()=>{const {leg,motion,s}=setup();s.mode='running';for(let i=0;i<200;i++){s.distance+=.04;motion.update(s,.01);}const before=leg.quaternion.clone();s.distance+=.4;motion.update(s,.1);assert.ok(before.angleTo(leg.quaternion)>.01);for(const index of [0,1]){s.mode='action';s.index=index;s.actionTime=.5;motion.update(s,.05);s.mode='running';s.distance+=.4;motion.update(s,.1);const landed=leg.quaternion.clone();s.distance+=.4;motion.update(s,.1);assert.ok(landed.angleTo(leg.quaternion)>.01);}motion.dispose();});
